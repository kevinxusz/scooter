...
#
# dgd2.pm -- routines for configuring dgd v2
#

package Cons::Toolset::dgd2;
...
sub install_dir {
    my ($class, %options) = @_;
    my $install_path =  libfind( 'pattern' => '^dgd', 
				 'dirs' => [ cwd() ] );

    if( !$install_path ) {
	say( "error: can't find dgd", %options );
	return undef;
    }
...
sub basic_ruleset {
    ...
    my $ruleset = {
	'DGD_ROOT' => $install_dir,
	'DGD_INCLUDE' => '%DGD_ROOT/blib/include'
    };
