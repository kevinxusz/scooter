...
// 
// boxfish_cfg.cpp -- configuration facility, constructs a simple XML 
//                    configuration file
// 
...
static const QString REG_CONF_LOCATION("Boxfish3d\\CONFPATH");
static const QString REG_CONF_VERSION("Boxfish3d\\CONFVERS");
static const QString PATH_CONF_NAME("Boxfish3d/config.xml");
...
Config::init(boost::program_options::command_line_parser &parser) {
   namespace po = boost::program_options;
   po::options_description desc("Boxfish3d Command Line Options");
   desc.add_options()
   ...
   if(vm.count("config-path") > 0) {
      this->relocate( 
         QString::fromStdString(vm["config-path"].as<std::string>()) 
      );
   } else {
...
void Config::revert() {
   ...
   m_doc = new QDomDocument( "boxfish" );
...
void Config::load_defaults() {
   ...
   QFile xml_file( ":/boxfish_cfg_def.xml" );
   rc = xml_file.open( QIODevice::ReadOnly | QIODevice::Text );
   if( !rc ) {
